...
@Component
public class DeleteOffidocTemplateValidator implements IpasValidator<String> {
    ...
    public List<ValidationError> validate(String offidocType, Object... additionalArgs) {
        ...
        rejectIfEmptyString(result, offidocType, "delete.offidoc.template", "missing.offidoc.type");
        rejectIfEmptyString(result, template, "delete.offidoc.template", "missing.offidoc.template");

        COffidocType cOffidocType = offidocTypeService.selectById(offidocType);
        if (cOffidocType == null) {
            result.add(ValidationError.builder().pointer("delete.offidoc.template").messageCode("missing.offidoc.type").build());
        } else {
            ...
            if (Objects.isNull(deleteTemplate)) {
                result.add(ValidationError.builder().pointer("delete.offidoc.template").messageCode("missing.offidoc.template").build());
            } else {
                if (cOffidocType.getDefaultTemplate().equals(template)) {
                    result.add(ValidationError.builder().pointer("delete.offidoc.template").messageCode("delete.offidoc.default.template.error").build());
                }
            }
        }
