...
@Component
public class DeleteInvalidatedUserdocTypeValidator implements IpasValidator<String> {
    @Autowired
    ...
    public List<ValidationError> validate(String userdocType, Object... additionalArgs) {
        ...
        if (userdocType == null) {
            result.add(ValidationError.builder().pointer("invalidated.userdoc.type.delete").messageCode("missing.userdoc.type").build());
        }
        if (invaliteUserdocType == null) {
            result.add(ValidationError.builder().pointer("invalidated.userdoc.type.delete").messageCode("missing.invalidate.userdoc.type").build());
        }
