...
IF EXISTS(SELECT * FROM sys.triggers WHERE type = 'TR' AND name = 'SYNC_ABDOCS_PERSON_AFTER_UPDATE') ALTER TABLE IPASPROD.IP_PERSON DISABLE TRIGGER SYNC_ABDOCS_PERSON_AFTER_UPDATE;

--changeset dveizov:209.2
UPDATE IPASPROD.IP_PERSON SET PERSON_NAME = REPLACE(PERSON_NAME, char(13), '') where PERSON_NAME like '%' + char(13) + '%';
UPDATE IPASPROD.IP_PERSON SET PERSON_NAME = REPLACE(PERSON_NAME, char(10), '') where PERSON_NAME like '%' + char(10) + '%';
UPDATE IPASPROD.IP_PERSON SET PERSON_NAME = LTRIM(RTRIM(PERSON_NAME)) WHERE RIGHT(PERSON_NAME,1) = char(32) OR LEFT(PERSON_NAME,1) = char(32);

--changeset dveizov:209.3
...
IF EXISTS(SELECT * FROM sys.triggers WHERE type = 'TR' AND name = 'SYNC_ABDOCS_PERSON_AFTER_UPDATE') ALTER TABLE IPASPROD.IP_PERSON ENABLE TRIGGER SYNC_ABDOCS_PERSON_AFTER_UPDATE;
