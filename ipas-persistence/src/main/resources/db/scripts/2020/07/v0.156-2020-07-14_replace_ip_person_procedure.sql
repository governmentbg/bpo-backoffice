...
--changeset dveizov:156 splitStatements:false
...
BEGIN
    BEGIN TRY
        BEGIN TRANSACTION
            ...
            IF @selectedOldPersonRecordCount <> 1 -- Check if action exists
                BEGIN
            ...
            DECLARE @oldPersonAddressesCount int;
            SET @oldPersonAddressesCount = (
                SELECT COUNT(*) FROM IPASPROD.IP_PERSON_ADDRESSES a WHERE a.PERSON_NBR = @oldPersonNumber
            );

            PRINT 'Delete IPASPROD.IP_PERSON_ADDRESS: PERSON_NBR = '+ CAST(@oldPersonNumber as varchar)+', ADDR_NBR = ' +CAST(@oldAddressNumber as varchar);
            DELETE FROM IPASPROD.IP_PERSON_ADDRESSES WHERE PERSON_NBR = @oldPersonNumber and ADDR_NBR = @oldAddressNumber

            IF @oldPersonAddressesCount < 2 -- Delete IPASPROD.IP_PERSON record
                BEGIN
                    PRINT 'Delete IPASPROD.IP_PERSON: PERSON_NBR = '+ CAST(@oldPersonNumber as varchar) ;
                    DELETE FROM IPASPROD.IP_PERSON WHERE PERSON_NBR = @oldPersonNumber
                END
        COMMIT TRANSACTION
    END TRY
    BEGIN CATCH
