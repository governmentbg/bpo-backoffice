...
--changeset vnikolov:213.1 splitStatements:false
/***********************************************************************************************************************/
...
BEGIN
	...
	IF EXISTS (SELECT * FROM inserted) AND EXISTS (SELECT * FROM deleted)
		AND (SELECT count(*) FROM
		        (SELECT [PROC_TYP], [PROC_NBR], [ACTION_NBR] FROM inserted
		        UNION
		        SELECT [PROC_TYP], [PROC_NBR], [ACTION_NBR] FROM deleted) u ) > 1
	BEGIN
		SET @IndexActivity = 'I';
		SET @DeletActivity = 'D';
	END
...
BEGIN
	...
	IF EXISTS (SELECT * FROM inserted) AND EXISTS (SELECT * FROM deleted)
		AND (SELECT count(*) FROM
		        (SELECT [FILE_SEQ], [FILE_TYP], [FILE_SER], [FILE_NBR] FROM inserted
		        UNION
		        SELECT [FILE_SEQ], [FILE_TYP], [FILE_SER], [FILE_NBR] FROM deleted) u ) > 1
	BEGIN
		SET @IndexActivity = 'I';
		SET @DeletActivity = 'D';
	END
...
BEGIN
    ...
    SELECT
		...
		@TriggerName
          FROM inserted i
		  JOIN [IPASPROD].[IP_PATENT] p ON p.[FILE_SEQ] = i.[FILE_SEQ]
		    AND p.[FILE_TYP] = i.[FILE_TYP]
		    AND p.[FILE_SER] = i.[FILE_SER]
		    AND p.[FILE_NBR] = i.[FILE_NBR]
	UNION
	SELECT
		...
		@TriggerName
          FROM inserted i
		  JOIN [IPASPROD].[IP_MARK] m ON m.[FILE_SEQ] = i.[FILE_SEQ]
		    AND m.[FILE_TYP] = i.[FILE_TYP]
		    AND m.[FILE_SER] = i.[FILE_SER]
		    AND m.[FILE_NBR] = i.[FILE_NBR]
    UNION
    SELECT
		...
		@TriggerName
          FROM deleted d
		  JOIN [IPASPROD].[IP_PATENT] p ON p.[FILE_SEQ] = d.[FILE_SEQ]
		    AND p.[FILE_TYP] = d.[FILE_TYP]
		    AND p.[FILE_SER] = d.[FILE_SER]
		    AND p.[FILE_NBR] = d.[FILE_NBR]
    UNION
    SELECT
        '2',
        d.FILE_SEQ,
		...
		@TriggerName
          FROM deleted d
		  JOIN [IPASPROD].[IP_MARK] m ON m.[FILE_SEQ] = d.[FILE_SEQ]
		    AND m.[FILE_TYP] = d.[FILE_TYP]
		    AND m.[FILE_SER] = d.[FILE_SER]
		    AND m.[FILE_NBR] = d.[FILE_NBR]
END
...
BEGIN
	...
	IF EXISTS (SELECT * FROM inserted) AND EXISTS (SELECT * FROM deleted)
		AND (SELECT count(*) FROM
		        (SELECT [DOC_ORI], [DOC_LOG], [DOC_SER], [DOC_NBR] FROM inserted
		        UNION
		        SELECT [DOC_ORI], [DOC_LOG], [DOC_SER], [DOC_NBR] FROM deleted) u ) > 1
	BEGIN
		SET @IndexActivity = 'I';
		SET @DeletActivity = 'D';
	END
    ...
    IF NOT EXISTS (SELECT *
        ...
        BEGIN
            ...
            FROM
                inserted i
                    JOIN [IPASPROD].[IP_USERDOC] ud ON ud.DOC_NBR = i.DOC_NBR
                        AND ud.DOC_ORI = i.DOC_ORI
                        AND ud.DOC_LOG = i.DOC_LOG
                        AND ud.DOC_SER = i.DOC_SER
            UNION
            SELECT
                '11',
                d.DOC_ORI,
            ...
            FROM
                deleted d
                    JOIN [IPASPROD].[IP_USERDOC] ud ON ud.DOC_NBR = d.DOC_NBR
                        AND ud.DOC_ORI = d.DOC_ORI
                        AND ud.DOC_LOG = d.DOC_LOG
                        AND ud.DOC_SER = d.DOC_SER;
        END
END
...
BEGIN
	...
	IF NOT EXISTS (SELECT *
			...
			JOIN inserted i ON iq.[DOC_ORI] = i.[DOC_ORI]
			    ...
			    AND iq.[ROLE] = i.[ROLE]
			WHERE iq.[OPERATION] = @IndexActivity AND iq.[INDEXED_AT] is null)
		AND NOT EXISTS(SELECT *
			FROM [EXT_CORE].[INDEX_QUEUE] iq
			JOIN deleted d ON iq.[DOC_ORI] = d.[DOC_ORI]
                AND iq.[DOC_LOG] = d.[DOC_LOG]
			    ...
			    AND iq.[ROLE] = d.[ROLE]
			WHERE iq.[OPERATION] = @DeletActivity AND iq.[INDEXED_AT] is null)
	BEGIN
		INSERT INTO [EXT_CORE].[INDEX_QUEUE](
			TYPE,
			DOC_ORI,
    ...
    IF NOT EXISTS (SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN inserted i ON iq.[DOC_ORI] = i.[DOC_ORI]
                                ...
                                AND iq.[DOC_NBR] = i.[DOC_NBR]
                   WHERE iq.[TYPE] = @Type
                        AND (iq.[OPERATION] = 'I' OR iq.[OPERATION] = 'I' )
                        AND iq.[INDEXED_AT] is null)
        AND
        NOT EXISTS(SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
        ...
        AND EXISTS(SELECT TOP (1) i.*
                   FROM inserted i
                            JOIN [IPASPROD].[IP_USERDOC] ud ON i.[DOC_ORI] = ud.[DOC_ORI]
                                AND i.[DOC_LOG] = ud.[DOC_LOG]
                                AND i.[DOC_SER] = ud.[DOC_SER]
                                AND i.[DOC_NBR] = ud.[DOC_NBR]
                   UNION
                   SELECT TOP (1) d.*
                   FROM deleted d
                            JOIN [IPASPROD].[IP_USERDOC] ud ON d.[DOC_ORI] = ud.[DOC_ORI]
                                AND d.[DOC_LOG] = ud.[DOC_LOG]
                                AND d.[DOC_SER] = ud.[DOC_SER]
                                AND d.[DOC_NBR] = ud.[DOC_NBR])
        BEGIN
            INSERT INTO [EXT_CORE].[INDEX_QUEUE](
                TYPE,
                DOC_ORI,
...
--changeset vnikolov:213.10 splitStatements:false
/***********************************************************************************************************************/
...
BEGIN
	...
	IF EXISTS (SELECT * FROM inserted) AND EXISTS (SELECT * FROM deleted)
		AND (SELECT count(*) FROM (
		    SELECT [ELEMENT_NBR] FROM inserted
		    UNION
		    SELECT [ELEMENT_NBR] FROM deleted) u ) > 1
	BEGIN
		SET @IndexActivity = 'I';
		SET @DeletActivity = 'D';
	END
	...
	END
     ...
     IF NOT EXISTS (SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN inserted i ON iq.[PROC_NBR] = i.ACTION_PROC_NBR
                                AND iq.[PROC_TYP] = i.ACTION_PROC_TYPE
                                AND iq.[ACTION_NBR] = i.ACTION_ACTION_NBR
                   WHERE iq.[TYPE] = @Type
                        AND iq.[OPERATION] = @IndexActivity
                        AND iq.[INDEXED_AT] is null )
        AND NOT EXISTS(SELECT *
                       FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN deleted d ON iq.[PROC_NBR] = d.ACTION_PROC_NBR
                                AND iq.[PROC_TYP] = d.ACTION_PROC_TYPE
                                AND iq.[ACTION_NBR] = d.ACTION_ACTION_NBR
                       WHERE iq.[TYPE] = @Type
                            AND iq.[OPERATION] = @DeletActivity
                            AND iq.[INDEXED_AT] is null )
        BEGIN
            ...
            FROM
                inserted
			WHERE ACTION_PROC_NBR IS NOT NULL
			    AND ACTION_PROC_TYPE IS NOT NULL
			    AND ACTION_ACTION_NBR IS NOT NULL
            UNION
            SELECT
                @Type,
                ACTION_PROC_TYPE,
    ...
    IF NOT EXISTS (SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN inserted i ON iq.[PROC_NBR] = i.ACTIONUDOC_PROC_NBR
                                AND  iq.[PROC_TYP] = i.ACTIONUDOC_PROC_TYPE
                                AND iq.[ACTION_NBR] = i.ACTIONUDOC_ACTION_NBR
                   WHERE iq.[TYPE] = @Type
                        AND iq.[OPERATION] = @IndexActivity
                        AND iq.[INDEXED_AT] is null)
        AND
        NOT EXISTS(SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN deleted d ON iq.[PROC_NBR] = d.ACTIONUDOC_PROC_NBR
                                AND  iq.[PROC_TYP] = d.ACTIONUDOC_PROC_TYPE
                                AND iq.[ACTION_NBR] = d.ACTIONUDOC_ACTION_NBR
                   WHERE iq.[TYPE] = @Type
                        AND iq.[OPERATION] = @DeletActivity
                        AND iq.[INDEXED_AT] is null)
        BEGIN
            ...
            FROM
                inserted
			WHERE ACTIONUDOC_PROC_NBR IS NOT NULL
			    AND ACTIONUDOC_PROC_TYPE IS NOT NULL
			    AND ACTIONUDOC_ACTION_NBR IS NOT NULL
            UNION
            SELECT
                @Type,
                ACTIONUDOC_PROC_TYPE,
...
--changeset vnikolov:213.11 splitStatements:false
/***********************************************************************************************************************/
...
--changeset vnikolov:213.12 splitStatements:false
/***********************************************************************************************************************/
...
--changeset vnikolov:213.13 splitStatements:false
/***********************************************************************************************************************/
...
--changeset vnikolov:213.14 splitStatements:false
/***********************************************************************************************************************/
...
--changeset vnikolov:213.15 splitStatements:false
/***********************************************************************************************************************/
    ...
    IF NOT EXISTS (SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN inserted i ON iq.[FILE_SEQ] = i.[FILE_SEQ]
                                ...
                                AND iq.[FILE_NBR] = i.[FILE_NBR]
                   WHERE iq.[TYPE] = @Type
                        AND (iq.[OPERATION] = 'I' OR iq.[OPERATION] = 'U')
                        AND iq.[INDEXED_AT] is null)
        AND
        NOT EXISTS(SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
...
--changeset vnikolov:213.16 splitStatements:false
/***********************************************************************************************************************/
...
--changeset vnikolov:213.17 splitStatements:false
/***********************************************************************************************************************/
...
--changeset vnikolov:213.18 splitStatements:false
/***********************************************************************************************************************/
...
BEGIN
    ...
    IF NOT EXISTS (SELECT *
        ...
        AND EXISTS(SELECT TOP (1) i.*
                        FROM inserted i
                        JOIN [IPASPROD].[IP_PATENT] p ON i.[FILE_SEQ] = p.[FILE_SEQ]
                            AND i.[FILE_TYP] = p.[FILE_TYP]
                            AND i.[FILE_SER] = p.[FILE_SER]
                            AND i.[FILE_NBR] = p.[FILE_NBR]
                  UNION
                  SELECT TOP (1) d.*
                        FROM deleted d
                        JOIN [IPASPROD].[IP_PATENT] p ON d.[FILE_SEQ] = p.[FILE_SEQ]
                            AND d.[FILE_TYP] = p.[FILE_TYP]
                            AND d.[FILE_SER] = p.[FILE_SER]
                            AND d.[FILE_NBR] = p.[FILE_NBR]
                  )
        BEGIN
            INSERT INTO [EXT_CORE].[INDEX_QUEUE](
                TYPE,
...
--changeset vnikolov:213.19 splitStatements:false
/***********************************************************************************************************************/
    ...
    IF NOT EXISTS (SELECT *
        ...
        AND EXISTS(
			  SELECT TOP (1) p.[FILE_SEQ]
                    FROM inserted ii
                    JOIN [IPASPROD].[IP_PATENT] p ON ii.[FILE_SEQ] = p.[FILE_SEQ]
                        AND ii.[FILE_TYP] = p.[FILE_TYP]
                        AND ii.[FILE_SER] = p.[FILE_SER]
                        AND ii.[FILE_NBR] = p.[FILE_NBR]
              UNION
              SELECT TOP (1) p.[FILE_SEQ]
                    FROM deleted dd
                    JOIN [IPASPROD].[IP_PATENT] p ON dd.[FILE_SEQ] = p.[FILE_SEQ]
                        AND dd.[FILE_TYP] = p.[FILE_TYP]
                        AND dd.[FILE_SER] = p.[FILE_SER]
                        AND dd.[FILE_NBR] = p.[FILE_NBR]
              )
        BEGIN
			INSERT INTO [EXT_CORE].[INDEX_QUEUE](
                TYPE,
...
BEGIN
    ...
    IF NOT EXISTS (SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN [EXT_CORE].[PLANT] p ON iq.[FILE_SEQ] = p.[FILE_SEQ]
                                AND iq.[FILE_TYP] = p.[FILE_TYP]
                                AND iq.[FILE_SER] = p.[FILE_SER]
                                AND iq.[FILE_NBR] = p.[FILE_NBR]
        ...
        AND EXISTS(SELECT TOP (1) i.*
                    ...
                    JOIN [IPASPROD].[IP_PATENT] pt ON pt.[FILE_SEQ] = p.[FILE_SEQ]
                        AND pt.[FILE_TYP] = p.[FILE_TYP]
                        AND pt.[FILE_SER] = p.[FILE_SER]
                        AND pt.[FILE_NBR] = p.[FILE_NBR]
              UNION
              SELECT TOP (1) d.*
                    FROM deleted d
...
BEGIN
    ...
    IF NOT EXISTS (SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN inserted i ON iq.[PROC_NBR] = i.[PROC_NBR]
                                AND iq.[PROC_TYP] = i.[PROC_TYP]
                   WHERE iq.[TYPE] = @Type
                        AND iq.[OPERATION] = @IndexActivity
                        AND iq.[INDEXED_AT] is null)
        AND
        NOT EXISTS(SELECT *
                   FROM [EXT_CORE].[INDEX_QUEUE] iq
                            JOIN deleted d ON iq.[PROC_NBR] = d.[PROC_NBR]
                                AND  iq.[PROC_TYP] = d.[PROC_TYP]
                   WHERE iq.[TYPE] = @Type
                        AND iq.[OPERATION] = @DeletActivity
                        AND iq.[INDEXED_AT] is null)
        BEGIN
            ...
            FROM [IPASPROD].[IP_PROC] p
                     JOIN inserted i ON i.[PROC_NBR] = p.[PROC_NBR]
                        AND i.[PROC_TYP] = p.[PROC_TYP]
                     JOIN [IPASPROD].[IP_USERDOC] ud ON ud.DOC_NBR = p.DOC_NBR
                        AND ud.DOC_ORI = p.DOC_ORI
                        AND ud.DOC_LOG = p.DOC_LOG
                        AND ud.DOC_SER = p.DOC_SER
            WHERE p.DOC_NBR is not null
                AND p.DOC_ORI is not null
                AND p.DOC_LOG is not null
                AND p.DOC_SER is not null
            UNION
            SELECT '11', p.DOC_NBR, p.DOC_ORI, p.DOC_LOG, p.DOC_SER, GETDATE(), 'I', @TriggerName
            FROM [IPASPROD].[IP_PROC] subp
                     JOIN [IPASPROD].[IP_PROC] p ON p.FILE_PROC_NBR = subp.PROC_NBR
                        AND p.FILE_PROC_TYP = subp.PROC_TYP
                     JOIN inserted i ON i.[PROC_NBR] = p.[PROC_NBR]
                        AND i.[PROC_TYP] = p.[PROC_TYP]
                     JOIN [IPASPROD].[IP_USERDOC] ud ON ud.DOC_NBR = p.DOC_NBR
                        AND ud.DOC_ORI = p.DOC_ORI
            ...
            SELECT '11', p.DOC_NBR, p.DOC_ORI, p.DOC_LOG, p.DOC_SER, GETDATE(), 'I', @TriggerName
            FROM [IPASPROD].[IP_PROC] p
                     JOIN deleted d ON d.[PROC_NBR] = p.[PROC_NBR]
                        AND d.[PROC_TYP] = p.[PROC_TYP]
                     JOIN [IPASPROD].[IP_USERDOC] ud ON ud.DOC_NBR = p.DOC_NBR
                        AND ud.DOC_ORI = p.DOC_ORI
                        AND ud.DOC_LOG = p.DOC_LOG
                        AND ud.DOC_SER = p.DOC_SER
            ...
            IF EXISTS(SELECT i.FILE_SEQ, i.FILE_TYP, i.FILE_SER, i.FILE_NBR
                      ...
                      FROM [IPASPROD].[IP_PATENT] p
                               JOIN deleted d ON p.[FILE_SEQ] = d.[FILE_SEQ]
                                    AND p.[FILE_TYP] = d.[FILE_TYP]
                                    AND p.[FILE_SER] = d.[FILE_SER]
                                    AND p.[FILE_NBR] = d.[FILE_NBR]
                      UNION
                      SELECT d.FILE_SEQ, d.FILE_TYP, d.FILE_SER, d.FILE_NBR
                      FROM [IPASPROD].[IP_MARK] m
                               JOIN deleted d ON m.[FILE_SEQ] = d.[FILE_SEQ]
                                    AND m.[FILE_TYP] = d.[FILE_TYP]
                                    AND m.[FILE_SER] = d.[FILE_SER]
                                    AND m.[FILE_NBR] = d.[FILE_NBR])
                BEGIN
                            ...
                            WHEN EXISTS (SELECT *
                                         FROM [IPASPROD].[IP_PATENT] p
                                                  JOIN inserted i ON p.[FILE_SEQ] = i.[FILE_SEQ]
                                                    AND p.[FILE_TYP] = i.[FILE_TYP]
                                                    AND p.[FILE_SER] = i.[FILE_SER]
                                                    AND p.[FILE_NBR] = i.[FILE_NBR]) THEN '1'
                            WHEN EXISTS (SELECT *
                                         FROM [IPASPROD].[IP_MARK] m
                                                  JOIN inserted i ON m.[FILE_SEQ] = i.[FILE_SEQ]
                                                    AND m.[FILE_TYP] = i.[FILE_TYP]
                                                    AND m.[FILE_SER] = i.[FILE_SER]
                                                    AND m.[FILE_NBR] = i.[FILE_NBR]) THEN '2'
                            ELSE '94'
                        END,
                        FILE_SEQ,
                        FILE_TYP,
                            ...
                            WHEN EXISTS (SELECT *
                                         FROM [IPASPROD].[IP_PATENT] p
                                                  JOIN deleted d ON p.[FILE_SEQ] = d.[FILE_SEQ]
                                                    AND p.[FILE_TYP] = d.[FILE_TYP]
                                                    AND p.[FILE_SER] = d.[FILE_SER]
                                                    AND p.[FILE_NBR] = d.[FILE_NBR]) THEN '1'
                            WHEN EXISTS (SELECT *
                                         FROM [IPASPROD].[IP_MARK] m
                                                  JOIN deleted d ON m.[FILE_SEQ] = d.[FILE_SEQ]
                                                    AND m.[FILE_TYP] = d.[FILE_TYP]
                                                    AND m.[FILE_SER] = d.[FILE_SER]
                                                    AND m.[FILE_NBR] = d.[FILE_NBR]) THEN '2'
                            ELSE '93'
                        END,
                        FILE_SEQ,
                        FILE_TYP,
