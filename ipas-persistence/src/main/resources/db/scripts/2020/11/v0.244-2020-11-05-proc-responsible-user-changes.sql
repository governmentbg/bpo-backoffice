...
--changeset ggeorgiev:244.1
create table EXT_CORE.IP_PROC_RESPONSIBLE_USER_CHANGES
(
    PROC_TYP varchar(4),
    PROC_NBR numeric(8),
    CHANGE_NBR integer,
    USER_CHANGED numeric(5),
    DATE_CHANGED datetime,
    OLD_RESPONSIBLE_USER_ID numeric(5),
    NEW_RESPONSIBLE_USER_ID numeric(5),
    STATUS integer,
    constraint IP_PROC_RESPONSIBLE_USER_CHANGES_PK
        primary key (PROC_TYP, PROC_NBR, CHANGE_NBR),
);


--changeset ggeorgiev:244.2 splitStatements:false
DROP PROCEDURE IPASPROD.deleteProcessAction;

--changeset ggeorgiev:244.3 splitStatements:false
...
--changeset dveizov:244.5 splitStatements:false
    ...
    BEGIN TRY
        BEGIN TRANSACTION
            ...
            IF (@userdocGroupName IS NOT NULL AND @userdocGroupName = 'CORRESP')
                BEGIN
                    DECLARE @oldProcessResponsibleUserId numeric(5);
                    SELECT @oldProcessResponsibleUserId = RESPONSIBLE_USER_ID FROM IPASPROD.IP_PROC p where p.PROC_TYP = @userdocProcTyp AND p.PROC_NBR = @userdocProcNbr;
                    ...
                    if ( @oldProcessResponsibleUserId is null and @newUpperProcessResponsibleUserId is not null) or (@oldProcessResponsibleUserId is not null and @newUpperProcessResponsibleUserId is null) or (@oldProcessResponsibleUserId != @newUpperProcessResponsibleUserId)
                    BEGIN
                        DECLARE @changeNbr int;
                        SET @changeNbr = (
                            SELECT coalesce(max(CHANGE_NBR) + 1, 1)
                            FROM EXT_CORE.IP_PROC_RESPONSIBLE_USER_CHANGES
                            where PROC_NBR = @userdocProcNbr
                              and PROC_TYP = @userdocProcTyp
                        );
                        INSERT INTO EXT_CORE.IP_PROC_RESPONSIBLE_USER_CHANGES (PROC_TYP, PROC_NBR, CHANGE_NBR, USER_CHANGED, DATE_CHANGED, OLD_RESPONSIBLE_USER_ID, NEW_RESPONSIBLE_USER_ID, STATUS)
                        VALUES (@userdocProcTyp, @userdocProcNbr, @changeNbr, @changeUserId, getdate(), @oldProcessResponsibleUserId, @newUpperProcessResponsibleUserId, 0);
                    END
                END
