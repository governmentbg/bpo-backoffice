...
--changeset dveizov:295 splitStatements:false
...
BEGIN
    ...
    IF (((@insertedPersonIndiTxt IS NULL OR @insertedPersonIndiTxt = '') AND @insertedPersonIndiType IS NOT NULL))
        BEGIN
            UPDATE IPASPROD.IP_PERSON SET INDI_PERSON_ID_NBR = NULL, INDI_PERSON_ID_TYP = NULL WHERE PERSON_NBR = @insertedPerson
        END

    IF (@insertedPersonIndiTxt IS NOT NULL AND (@insertedPersonIndiType IS NULL OR @insertedPersonIndiType = '') )
        BEGIN
