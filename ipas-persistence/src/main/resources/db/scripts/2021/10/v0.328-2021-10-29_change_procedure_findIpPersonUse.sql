...
--changeset mmurlev:328.1
drop procedure findIpPersonUse

--changeset mmurlev:328.2 splitStatements:false
CREATE PROCEDURE [IPASPROD].[findIpPersonUse] @personNumber numeric(8),
                                              ...
                                              @acpRepresentativesCount int OUTPUT,
                                              @acpInfringerCount int OUTPUT,
                                              @acpServicePersonCount int OUTPUT
AS
BEGIN
    BEGIN TRY
        BEGIN TRANSACTION
            ...
            SET @partnershipHistoryCount = (
              ...
              SET @acpRepresentativesCount = (
                SELECT count(*) FROM EXT_CORE.ACP_REPRS a where a.PERSON_NBR = @personNumber
            )

              SET @acpInfringerCount = (
                SELECT count(*) FROM EXT_CORE.ACP_INFRINGER a where a.INFRINGER_PERSON_NBR = @personNumber
            )

              SET @acpServicePersonCount = (
                SELECT count(*) FROM EXT_CORE.ACP_SERVICE_PERSON a where a.SERVICE_PERSON_NBR = @personNumber
            )
